...
function createEmbed({ title, description, color = 0x00bcd4, fields = [], footer, timestamp = true }) {
...
module.exports = {
  ...
  execute(message) {
    ...
    const generalCmds = formatCommands(
      commands.filter(cmd => !['callnote', 'note', 'memory', 'puissance4', 'calendar'].includes(cmd.name))
    ...
    const embed = createEmbed({
      ...
      fields: [
        ...
        { name: '📋 Commandes Callnote', value: formatCommands([
        ...
        { name: '📝 Commandes Note', value: formatCommands([
