...
module.exports = {
  name: "p4",
  description:
    "Joue à Puissance 4 ! Usage: start | <colonne> | reset | scoreboard",
  async execute(message, args) {
    ...
    if (sub === "board") {
      if (!game.board)
        return sendError(
          message,
          "Aucun jeu en cours",
          "Il n’y a pas de partie en cours dans ce salon.",
        );
      return message.channel.send({
        embeds: [renderGameBoardEmbed(game, message.author)],
      });
    }
    ...
    if (!args.length)
      return sendError(
        message,
        "Commande invalide",
        "Usage : `start | <colonne> | reset | scoreboard`",
      );
    ...
    if (!placed)
      return sendError(
        message,
        "Colonne pleine",
        `La colonne **${col + 1}** est déjà remplie.`,
      );
