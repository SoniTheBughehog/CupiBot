...
// Export pour intégration dans votre bot existant
module.exports = {
    name: "casino",
    description: "Système de casino complet avec machine à sous, roulette et blackjack",
    ...
    // ROULETTE
    async playRoulette(message, bet, choice) {
        const userId = message.author.id;
        const player = this.getPlayer(userId);
    ...
    // BLACKJACK
    async startBlackjack(message, bet) {
        const userId = message.author.id;
        const player = this.getPlayer(userId);
    ...
    async execute(message, args) {
        ...
        if (command === 'roulette') {
            ...
            if (!bet || !choice) {
                return message.reply('❌ Usage: `!casino roulette <mise> <choix>`\nChoix possibles: rouge, noir, pair, impair, ou un numéro (0-36)');
            }
            ...
            await this.playRoulette(message, bet, choice);
            return;
        }
        ...
        if (command === 'blackjack') {
            ...
            await this.startBlackjack(message, bet);
            return;
        }
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields(
                ...
                { name: '🎰 Machine à sous', value: '`!casino slots <mise>` - Jouer aux machines à sous\nMise: 1-100 jetons', inline: false },
                { name: '🎲 Roulette', value: '`!casino roulette <mise> <choix>`\nChoix: rouge, noir, pair, impair, 0-36\nMise: 1-200 jetons', inline: false },
                { name: '🃏 Blackjack', value: '`!casino blackjack <mise>` - Jouer au blackjack\nMise: 1-150 jetons', inline: false }
            )
            .setColor(0xff6b6b)
            .setFooter({ text: 'Vous commencez avec 500 jetons !' });
